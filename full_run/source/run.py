...
def main():
    ...
    parser.add_argument('--flow-file', dest='flow_file', action='store',
                        help="existing traffic file basename relative to config/, without .txt")
    parser.add_argument('--run-id', dest='run_id', action='store',
                        help="caller-supplied numeric run ID (default: random unused ID)")
    parser.add_argument('--history-file', dest='history_file', action='store',
                        default='./mix/.history', help="history file (default: ./mix/.history)")
    parser.add_argument('--skip-analysis', dest='skip_analysis', action='store_true',
                        help="skip repository FCT/queue analysis for an isolated external analyzer")
    parser.add_argument('--cwh-extra-reply-deadline-us', dest='cwh_extra_reply_deadline_us',
                        type=int, help="override ConWeave's extra reply-deadline term in us")
    ...
    if args.run_id is not None:
        if not args.run_id.isdigit() or int(args.run_id) >= MAX_RAND_RANGE:
            raise Exception("CONFIG ERROR: --run-id must be numeric and less than {}".format(MAX_RAND_RANGE))
        config_ID = args.run_id
        if os.path.exists(os.getcwd() + "/mix/output/" + config_ID):
            raise Exception("CONFIG ERROR: run ID {} already exists".format(config_ID))
    else:
        isExist = True
        config_ID = 0
        while (isExist):
    ...
    if args.flow_file is not None:
        flow = args.flow_file[:-4] if args.flow_file.endswith('.txt') else args.flow_file
        config_root = os.path.realpath(os.path.join(os.getcwd(), "config"))
        flow_path = os.path.realpath(os.path.join(config_root, flow + ".txt"))
        if not flow_path.startswith(config_root + os.sep):
            raise Exception("CONFIG ERROR: --flow-file must remain under config/")
        if not exists(flow_path):
            raise Exception("CONFIG ERROR: custom flow file does not exist: {}".format(flow_path))
        print("Using custom input traffic file: {}".format(flow_path))
    else:
    ...
    if (lb_mode == 9):
        ...
        if args.cwh_extra_reply_deadline_us is not None:
            if args.cwh_extra_reply_deadline_us <= 0:
                raise Exception("CONFIG ERROR: ConWeave reply-deadline override must be positive")
            cwh_extra_reply_deadline = args.cwh_extra_reply_deadline_us
    else:
        if args.cwh_extra_reply_deadline_us is not None:
            raise Exception("CONFIG ERROR: reply-deadline override is only valid for ConWeave")
    ...
    print(run_command)
    simulation_status = os.system("./waf --run 'scratch/network-load-balance {config_name}' > {output_log} 2>&1".format(
        config_name=config_name, output_log=output_log))
    if simulation_status != 0:
        print("Simulation failed with os.system status {}".format(simulation_status), file=sys.stderr)
        sys.exit(1)

    if args.skip_analysis:
        print("Skipping default analysis; external analysis requested.")
        print("\n\n============== Done ============== ")
        return
    ...
    fct_analysis_status = os.system("python3 fctAnalysis.py -id {config_ID} -dir {dir} -bdp {bdp} -sT {fct_analysis_time_limit_begin} -fT {fct_analysistime_limit_end} > /dev/null 2>&1".format(
        config_ID=config_ID, dir=os.getcwd(), bdp=bdp, fct_analysis_time_limit_begin=fct_analysis_time_limit_begin, fct_analysistime_limit_end=fct_analysistime_limit_end))
    if fct_analysis_status != 0:
        print("FCT analysis failed with os.system status {}".format(fct_analysis_status), file=sys.stderr)
        sys.exit(1)
    ...
    if lb_mode == 9: # ConWeave Logging
        ...
        queue_analysis_status = os.system("python3 queueAnalysis.py -id {config_ID} -dir {dir} -sT {queue_analysis_time_limit_begin} -fT {queue_analysis_time_limit_end} > /dev/null 2>&1".format(
            config_ID=config_ID, dir=os.getcwd(), queue_analysis_time_limit_begin=queue_analysis_time_limit_begin, queue_analysistime_limit_end=queue_analysistime_limit_end,
            monitoringInterval=sw_monitoring_interval))  # TODO: parameterize
        if queue_analysis_status != 0:
            print("Queue analysis failed with os.system status {}".format(queue_analysis_status), file=sys.stderr)
            sys.exit(1)
